...
export default function StudentDashboard() {
  ...
  const statCards = [
    { label: "Pending", value: stats.pending, icon: Clock, color: "text-blue-500", bg: "bg-blue-500/10", status: "todo" as Status },
    { label: "Completed", value: stats.completed, icon: CheckCircle, color: "text-green-500", bg: "bg-green-500/10", status: "completed" as Status },
    { label: "Overdue", value: stats.overdue, icon: AlertTriangle, color: "text-red-500", bg: "bg-red-500/10", status: "overdue" as Status },
    { label: "Total", value: stats.total, icon: BookOpen, color: "text-purple-500", bg: "bg-purple-500/10", status: "total" as const },
  ];

  const filteredAssignments = filter === null || filter === "total" 
    ? stats.upcoming 
    : assignments.filter(a => a.status === filter).slice(0, 5);

  const getFilterTitle = () => {
    if (!filter || filter === "total") return "Upcoming Deadlines";
    return `${filter.charAt(0).toUpperCase() + filter.slice(1)} Assignments`;
  };
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {statCards.map((stat, i) => (
            ...
            >
              <Card className={`border-border hover:shadow-md transition-all ${filter === stat.status ? 'ring-2 ring-primary bg-primary/5' : ''}`}>
              ...
              <div className="space-y-4">
                {loading ? (
                  Array.from({ length: 3 }).map((_, i) => (
                    <div key={i} className="h-20 w-full bg-secondary/20 animate-pulse rounded-xl" />
                  ))
                ) : filteredAssignments.length === 0 ? (
                  <div className="text-center py-12">
                    <p className="text-muted-foreground italic">No assignments found for this category.</p>
                    {filter && filter !== "total" && (
                      <Button variant="link" onClick={() => setFilter(null)} className="mt-2">Show all upcoming</Button>
                    )}
                  </div>
                ) : (
                    ...
                    {filteredAssignments.map((a, i) => (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground mt-0.5">{a.subject} · Due {format(new Date(a.due_date), "PPP")}</p>
                        </div>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-2">
                  <p className="text-xs opacity-70">
                    You've completed {stats.completed} assignments so far. Keep up the great momentum!
                  </p>
                </div>
